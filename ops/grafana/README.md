...
| **Per-trip** | Top trips by ops (table), per-trip live connections, per-trip op append rate. The hub exports per-trip series for only the **busiest `SIANO_METRICS_TOP_TRIPS` trips** (default 10) — the totals above cover the rest — so these panels always show a small, fixed set regardless of how many trips exist. |
