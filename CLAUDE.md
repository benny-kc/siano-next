...
| `client/js/ui/i18n.js` | Per-device UI language (localStorage `siano:lang`, default `auto` = follow the browser via `navigator.languages`). Client-side only — **the hub never sees a locale and never serves translated strings** (it's a dumb relay with no business logic); every language ships as a static catalog and the device picks one. `t(key, params)` translates a JS-rendered string (active catalog → English fallback → the key). `applyI18n()` writes `<html lang>`/`<html dir>` (`rtl` for Arabic) and localizes the static chrome in `index.html`: elements tagged `data-i18n` / `data-i18n-html` / `data-i18n-attr` are snapshotted (English is their source of truth, inline) and swapped for a non-English locale — English restores the snapshot, so no prose is duplicated. `setLocalePref(pref)` persists + applies; `app.js`'s `actions.setLocale` then repaints so board strings re-read `t()`. The **op-log is never translated** — meal/member/trip names are user data. The default traveller name (`Traveller N`) is localized to the creator's locale before it becomes an op. Money stays integer-cents (`core/money.js`); only the on-card month label and CSV timestamp follow the locale via `Intl`. |
| `client/js/i18n/*.js` | Message catalogs. `index.js` is the registry: `LOCALES` (`{ code, label, endonym, dir }` for the 11 shipped languages — en/es/fr/de/it/pt/zh/ja/ko/ar/pl) + `CATALOGS` (code → catalog), imported statically so they're in the offline shell and the asset-hash graph stays acyclic. `en.js` is the **source of truth for JS-rendered strings** (values are plain strings with `{placeholder}` tokens, or functions of `params` for plurals/grammar). **All 11 are fully translated** (273 keys each): `en` + `pl` are primary; the other nine are **machine-drafted pending native review** (see each file's header). Each language encodes its own plural rule where the count matters: `pl` a `plu(n, one, few, many)` (three forms), `es/de/it/pt` two forms, `fr` the CLDR 0–1 rule, `zh/ja/ko` a single form (counted strings just interpolate an invariant counter), and `ar` a six-category `arPlu(n, {zero,one,two,few,many,other})`. A missing key still falls back to English, so a partial edit is always safe. `test/i18n.test.mjs` guards that every `t()` key the UI uses exists in `en.js` and that no catalog carries an unknown key (run it after editing any catalog). NB: Arabic strings are done and `dir="rtl"` is wired, but full **RTL layout** polish in `app.css` is still a follow-up. |
...
| `hub/metrics.js` | `Metrics` (lifetime counters the hub bumps) + `render(metrics, live)` — Prometheus text exposition served at `GET /metrics`. **Token-gated (`SIANO_METRICS_TOKEN`), off (404) when unset** — series leak trip ids/volume. Covers client traffic, per-trip series, process, AND the **peer link** (`siano_peer_*`: link up/down, ops in/out, reconnects, inbound conns, auth failures). **Per-trip series are a bounded top-N** (`SIANO_METRICS_TOP_TRIPS`, default 10): the global totals cover every trip, so the hub emits a series only for the busiest few and retains a per-trip append counter for at most `maxTripSeries` trips (the least-active is dropped when full) — the old code kept one per trip for the life of the process, growing both hub memory and Prometheus cardinality without bound. Dependency-free; scrape it with Grafana Alloy/Agent → Grafana Cloud (see docs/security.md → *Metrics / monitoring*). NB the peer counter Map fields (`peerOpsIn`/`peerOpsOut`) and the record methods (`peerRecvOps`/`peerSentOps`) are deliberately named differently — a same-named instance field would shadow the prototype method. |
...
`/metrics`: 404 when off, bearer auth, live + per-trip series), `i18n` (the
localization engine: active/English fallback + `{placeholder}`/function values,
`navigator.languages` → locale resolution, LOCALES/CATALOGS consistency, and a
guard that every `t()` key the UI uses exists in `en.js`), `loadsim` (the
