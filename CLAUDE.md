...
| `client/js/i18n/*.js` | Message catalogs. `index.js` is the registry: `LOCALES` (`{ code, label, endonym, dir }` for the 10 shipped languages — en/es/fr/de/it/pt/zh/ja/ko/ar) + `CATALOGS` (code → catalog), imported statically so they're in the offline shell and the asset-hash graph stays acyclic. `en.js` is the **source of truth for JS-rendered strings** (values are plain strings with `{placeholder}` tokens, or functions of `params` for plurals/grammar). The other nine are **empty stubs today** (a missing key falls back to English) — shipping a language is purely additive: fill in `<code>.js` with the `en.js` keys + the `index.html` `data-i18n` keys, keeping tokens/tags intact. `test/i18n.test.mjs` guards that every `t()` key used exists in `en.js` and that no catalog carries an unknown key. NB: full **RTL layout** polish in `app.css` for Arabic is a follow-up beyond this string scaffold (`dir="rtl"` is already wired). |
