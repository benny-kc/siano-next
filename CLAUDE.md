...
| `client/js/i18n/*.js` | Message catalogs. `index.js` is the registry: `LOCALES` (`{ code, label, endonym, dir }` for the 11 shipped languages — en/es/fr/de/it/pt/zh/ja/ko/ar/**pl**) + `CATALOGS` (code → catalog), imported statically so they're in the offline shell and the asset-hash graph stays acyclic. `en.js` is the **source of truth for JS-rendered strings** (values are plain strings with `{placeholder}` tokens, or functions of `params` for plurals/grammar). **`pl.js` (Polish) is fully translated** — the reference for a plural-rich language: it defines a `plu(n, one, few, many)` helper (Polish has three plural forms — 1 / 2–4 / 5+ & teens) and routes every counted string (bills, people, ops, budgets, draft note…) through it. The other eight are **empty stubs** (a missing key falls back to English) — shipping a language is purely additive: fill in `<code>.js` with the `en.js` keys + the `index.html` `data-i18n` keys, keeping tokens/tags intact (a counted string becomes a function). `test/i18n.test.mjs` guards that every `t()` key used exists in `en.js` and that no catalog carries an unknown key. NB: full **RTL layout** polish in `app.css` for Arabic is a follow-up beyond this string scaffold (`dir="rtl"` is already wired). |
