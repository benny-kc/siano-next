...
| `client/js/ui/typography.js` | Per-device Appearance preferences (localStorage `siano:type`): font family/size/boldness and **light/dark theme**, applied as CSS vars + `data-siano-bold` / `data-siano-theme` on `<html>` (also syncs the `theme-color` meta). System-font stacks only (offline/CSP-safe). The light theme is a warm-beige palette defined once as token overrides under `:root[data-siano-theme="light"]` in `css/app.css`. |
